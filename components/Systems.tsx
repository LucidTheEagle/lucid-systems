...
export default function Systems() {
  const systems = [
    ...
    },
    {
      ...
      featured: true, // CENTER CARD - FEATURED
    },
    {
      id: "overwatch",
      title: "THE OVERWATCH",
  ...
  return (
    <section id="systems" className="relative w-full bg-obsidian py-24 md:py-32 overflow-hidden">
      ...
      {/* BACKGROUND GRID - Whisper-quiet blueprint feel */}
      <div className="absolute inset-0 bg-[linear-gradient(to_right,#80808014_1px,transparent_1px),linear-gradient(to_bottom,#80808014_1px,transparent_1px)] bg-[size:32px_32px] [mask-image:radial-gradient(ellipse_60%_50%_at_50%_50%,#000_70%,transparent_100%)]" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
          {systems.map((system, idx) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className={`group relative flex flex-col h-full bg-basalt p-8 md:p-10 transition-all duration-300 
                ${system.featured 
                  ? "md:-mt-8 md:pb-12 border border-lucid/30 shadow-[0_0_30px_rgba(0,240,255,0.1)] hover:shadow-[0_0_50px_rgba(0,240,255,0.2)]" 
                  : "border border-white/5 hover:border-lucid/50 hover:-translate-y-2 hover:shadow-[0_0_20px_rgba(0,240,255,0.15)]"
                }
              `}
            >
              {/* THE PHOTON BEAM - Sentry Only (Active Monitoring) */}
              {system.featured && (
              ...
              <div className="flex items-start justify-between mb-8">
                ...
                {system.featured && (
                  <span className="px-3 py-1 border border-lucid text-lucid text-[10px] font-bold uppercase tracking-widest bg-lucid/10 shadow-[0_0_10px_rgba(0,240,255,0.2)]">
                    Most Deployed
                  </span>
                )}
              </div>
              ...
              {/* TITLES */}
              <div className="mb-6">
                <h3 className="text-ancient text-xl md:text-2xl font-bold text-alabaster mb-2 tracking-[0.1em] uppercase group-hover:text-white transition-colors">
                  {system.title}
                </h3>
              ...
              {/* DESCRIPTION */}
              <p className="text-modern text-granite text-sm md:text-base leading-relaxed mb-8 border-l-2 border-white/5 pl-4 group-hover:border-lucid/50 transition-colors">
                {system.description}
              </p>
